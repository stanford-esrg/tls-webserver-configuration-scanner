...
func convertShToPsh (sh []signatureAndHash) []PsignatureAndHash {
	new_sh := make([]PsignatureAndHash, len(sh))
        for i:= 0; i < len(sh); i++ {
                sigHash := sh[i]
                new_sh[i] = PsignatureAndHash{sigHash.signature, sigHash.hash}
        }
        return new_sh
}

func (e *SignatureAlgorithmExtension) WriteToConfig(c *Config) error {
	c.SignatureAndHashes = convertShToPsh(e.getStructuredAlgorithms())
	return nil
}
