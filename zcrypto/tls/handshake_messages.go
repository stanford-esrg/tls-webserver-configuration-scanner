...
func (m *clientHelloMsg) marshal() []byte {
	...
	// Must deal with version constants in 
	// zcrypto/tls/common.go being type int
	tls13 := uint16(VersionTLS13)
	tls12 := uint16(VersionTLS12)
	...
	if numExtensions > 0 {
		...
		//Hardcoded special case for TLS 1.3
		if m.vers == tls13 {
			extensionsLength -= 1
		}
	...
	//Make space for required keyshare bytes
	if m.vers == tls13 {
		length += 42
		extensionsLength += 42
	}
	...
	// TLS 1.3 Handshakes have 1.2 in the version
	if m.vers == tls13 {
		x[4] = uint8(tls12 >> 8)
		x[5] = uint8(tls12)
	} else {
	...
	if m.multiStapling {
		// RFC 6961
		z[0] = byte(extensionStatusRequestV2 >> 8)
		z[1] = byte(extensionStatusRequestV2)
		z[2] = 0
		z[3] = 16
		z[4] = 0
		z[5] = 14
	...
	//Supported Version extension bytes
	if m.vers == tls13 {
		z[0] = byte(extensionSupportedTLSVersions >> 8)
		z[1] = byte(extensionSupportedTLSVersions & 0xff)
                z[2] = 0
                z[3] = 3
                z[4] = 2
                z[5] = byte(VersionTLS13 >> 8)
                z[6] = byte(VersionTLS13 & 0xff)
                z = z[7:]
	}
	if len(m.unknownExtensions) > 0 {
		for _, ext := range m.unknownExtensions {
...
func (m *serverHelloMsg) unmarshal(data []byte) bool {
	...
	for len(data) != 0 {
		...
		"illegal_parameter" alert.<Paste>

		*/
		case extensionSupportedTLSVersions:
			if length != 2 {
				return false
			}
			d := data[:length]
			m.supportedVersions = uint16(d[0])<<8 | uint16(d[1])
		case extensionNextProtoNeg:
			m.nextProtoNeg = true
			d := data[:length]
			for len(d) > 0 {
