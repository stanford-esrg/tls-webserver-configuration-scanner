...
func (c *Conn) clientHandshake() error {
	...
	if c.config.ClientFingerprintConfiguration != nil {
		...
		hello = &clientHelloMsg{
			...
			compressionMethods:   c.config.compressMethods(),
			random:               make([]byte, 32),
			ocspStapling:         !c.config.NoStapling,
			multiStapling:        c.config.MultiStapling,
			serverName:           c.config.ServerName,
			supportedCurves:      c.config.curvePreferences(),
			supportedPoints:      []uint8{pointFormatUncompressed},
			nextProtoNeg:         len(c.config.NextProtos) > 0,
			secureRenegotiation:  !c.config.NoSecureRenegotiation,
