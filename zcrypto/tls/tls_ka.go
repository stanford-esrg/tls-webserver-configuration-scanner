...
type auxSignatureAndHash struct {
	SignatureSchemeName string `json:"signature_scheme_name"`
	Hex                string `json:"hex"`
...
func (sh *SignatureAndHash) MarshalJSON() ([]byte, error) {
	aux := auxSignatureAndHash{
		SignatureSchemeName: nameForSignatureScheme(uint16(sh.hash)<<8+uint16(sh.signature)),
		Hex: "0x"+hex.EncodeToString([]byte{byte(sh.hash), byte(sh.signature)}),
