...
type TLSFlags struct {
	StartDelay             int  `long:"start-delay" description:"Number of seconds to delay the defaults scan." json:"start-delay"`
	HandshakeDelay         int  `long:"handshake-delay" description:"Number of seconds to delay each handshake in the defaults scan." json:"handshake_delay"`
	SessionResumptionDelay int  `long:"session-resumption-delay" description:"Number of seconds to delay between session resumption in the defaults scan." json:"session_resumption_delay"`
	...
	NoStapling            bool `long:"no-stp" description:"Do not send support for stapling" json:"no_stp"`
	NoSecureRenegotiation bool `long:"no-sr" description:"Do not send support for secure_renegotiation" json:"no_sr"`
	MultiStapling         bool `long:"multi-stp" description:"Send support for OCSP multi-stapling (status_request_v2)" json:"multi_stp"`
	...
	CompressMethod      string `long:"compress-method" description:"A comma-delimited list of compression methods to advertise."`
	ForceSuites      bool   `long:"force-suite" description:"Force ciphersuites in handshake."`
	...
	SessionCache tls.ClientSessionCache `json:"client_session_cache,omitempty"`

	SessionID []byte `json:"session-ID,omitempty"`
}
...
func (t *TLSFlags) GetTLSConfigForTarget(target *ScanTarget) (*tls.Config, error) {
	...
	if t.SessionCache != nil {
		ret.ClientSessionCache = t.SessionCache
	}

	if len(t.SessionID) > 0 {
		ret.SessionID = t.SessionID
	}
	...
	if t.StartDelay != 0 {
		ret.StartDelay = t.StartDelay
	} else {
		ret.StartDelay = 0
	}

	if t.HandshakeDelay != 0 {
		ret.HandshakeDelay = t.HandshakeDelay
	} else {
		ret.HandshakeDelay = 0
	}

	if t.SessionResumptionDelay != 0 {
		ret.SessionResumptionDelay = t.SessionResumptionDelay
	} else {
		ret.SessionResumptionDelay = 0
	}
	...
	if t.NoSecureRenegotiation {
		ret.NoSecureRenegotiation = true
	}
	if t.MultiStapling {
		ret.MultiStapling = true
	}
	...
	if t.CipherSuite != "" {
        ...
        if t.CompressMethod != "" {
                strMethods := getCSV(t.CompressMethod)
                var intMethods = make([]uint8, len(strMethods))
                for i, s := range strMethods {
                        s = strings.TrimPrefix(s, "0x")
                        v64, err := strconv.ParseUint(s, 16, 8)
                        if err != nil {
                                log.Fatalf("compression method: unable to convert %s to a 8bit integer: %s", s, err)
                        }
                        intMethods[i] = uint8(v64)
                }
                ret.CompressMethods = intMethods
        }
	...
	if t.CurvePreferences != "" {
		strCurves := getCSV(t.CurvePreferences)
		intCurves := make([]tls.CurveID, len(strCurves))
		for i, c := range strCurves {
			curveID, err := strconv.Atoi(c)
			if err != nil {
				log.Fatalf("curve preferences: unable to convert %s to a integer: %s", curveID, err)
			}
			intCurves[i] = tls.CurveID(uint16(curveID))
		}
		ret.CurvePreferences = intCurves
	}
	...
	if t.SignatureAlgorithms != "" {
		strAlgorithms := getCSV(t.SignatureAlgorithms)
		var intAlgorithms = make([]tls.PsignatureAndHash, len(strAlgorithms))
		for i, sighash := range strAlgorithms {
			s := strings.Split(sighash, "-")
			//https://tools.ietf.org/html/rfc8446#page-41
			//https://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml#tls-parameters-16
			signature_v64, err1 := strconv.ParseUint(s[1], 16, 16)
			hash_v64, err2 := strconv.ParseUint(s[0], 16, 16)
			if err1 != nil || err2 != nil {
				log.Fatalf("signature algorithms: unable to convert %s to a 8bit integer: %s", s, err)
			}
			intAlgorithms[i] = tls.PsignatureAndHash{uint8(signature_v64), uint8(hash_v64)}
		}
		ret.SignatureAndHashes = intAlgorithms
	}
...
const TLS_TIMEOUT time.Duration = time.Duration(10*time.Minute)
...
func (t *TLSFlags) GetTLSConnectionForTarget(conn net.Conn, target *ScanTarget) (*TLSConnection, error) {
	...
	tlsClient.SetDeadline(time.Now().Add(TLS_TIMEOUT))
