...
func (scanner *Scanner) scansSMTP(target zgrab2.ScanTarget) (*ScanResults, error) {
	...
	if scanner.config.StartTLS {
		...
		if code < 200 || code >= 300 {
			return result, fmt.Errorf("SMTP error code %d returned from STARTTLS command (%s)", code, ret)
		}
...
func (scanner *Scanner) Scan(target zgrab2.ScanTarget) (zgrab2.ScanStatus, interface{}, error) {
	plainResults, plainErr := scanner.scansSMTP(target)
	inSS, inHS, inSR := scanner.checkCipherSupport(target, scanner.config.BaseFlags, scanner.config.TLSFlags, def.INSECURE_CIPHERS)
	secSS, secHS, secSR := scanner.checkCipherSupport(target, scanner.config.BaseFlags, scanner.config.TLSFlags, def.SECURE_CIPHERS)
	ssl2SS, ssl2HS, ssl2SR := scanner.checkSSLv2Support(target, scanner.config.BaseFlags, scanner.config.TLSFlags)

	return zgrab2.SCAN_SUCCESS, SmtpTLSSupport{
		SupportsInsecure: inSS,
		SupportsSecure: secSS,
		SupportsSSL2: ssl2SS,
		InsecureHandshake: inHS,
		SecureHandshake: secHS,
		SSL2Handshake: ssl2HS,
		InsecureScanResults: inSR,
		SecureScanResults: secSR,
		SSL2ScanResults: ssl2SR,
		PlainError: def.Err{plainErr},
		PlainScanResults: plainResults,
	}, nil
